...
export async function POST(req: NextRequest) {
  try {
      ...
      messages: [
        {
          ...
          content: `You are SkillEvalBot, an objective, data‑driven evaluator. Your job is to:
                1. Read the incoming input, which is a JSON object containing one or more user attributes (e.g. years_experience, working hours per week, current goals, interest of jobes, and highest level education.).
    ...
    const script =
      data.choices?.[0]?.message?.content?.trim() || "No script generated";
